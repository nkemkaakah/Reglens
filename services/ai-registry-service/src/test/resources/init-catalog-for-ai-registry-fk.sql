-- Testcontainers Postgres init: minimal `catalog` schema + seed so ai-registry Flyway cross-schema FKs validate.
-- Mirrors catalog-service Flyway V1–V5 shape (teams, systems, controls, control_to_system + seed rows).
...
CREATE TABLE catalog.systems (
    ...
    tech_stack      TEXT[],
    repo_url        TEXT,
    owner_team_id   UUID REFERENCES catalog.teams (id),
...
CREATE TABLE catalog.control_to_system (
...
INSERT INTO catalog.teams (id, name, domain, email) VALUES
...
INSERT INTO catalog.systems (id, ref, display_name, description, domain, tech_stack, repo_url, owner_team_id, criticality) VALUES
    (
...
INSERT INTO catalog.controls (id, ref, category, title, description, evidence_type, review_frequency, owner_team_id, status) VALUES
    (
...
INSERT INTO catalog.control_to_system (control_id, system_id, notes) VALUES
    (
        'c1000000-0000-0000-0000-000000000004',
        'b1000000-0000-0000-0000-000000000001',
